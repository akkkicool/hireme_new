...
class EmailTemplateController extends Controller{
    ...
    protected $slug         =   'email-templates';
    ...
    public function index(Request $request){
        ...
        return view('admin.email_templates.index',compact('counter','records'));
    }

    public function view($id,Request $request){
        $record    =   EmailTemplate::where("id",base64_decode($id))->first();
        if(!empty($record)){
            return view('admin.email_templates.view',compact('record'));
        }else{
    ...
    public function edit($id,Request $request){
        if($request->isMethod('post')){
            $messages   =   [
                ...
                'subject.regex' => "The :attribute may only contain letters, numbers and spaces.",
            ];
            ...
            $exists_error = [];
            $exists   =   EmailTemplate::where("id","!=",base64_decode($id))->where("title",$request['title'])->where("is_deleted",0)->get();
    ...
    public function create(Request $request){
        if($request->isMethod('post')){
            ...
            /** Check Exists Work Start **/
            $exists_error = [];
            $exists   =   EmailTemplate::where("title",$request['title'])->where("is_deleted",0)->get();
            if($exists->count() > 0){ $exists_error["title"]   =  ["The title has already been taken"];  }
            if(!empty($exists_error)){
                return redirect()->back()->withInput()->withErrors($exists_error);
            }
    ...
    public function delete($id,Request $request){
        EmailTemplate::where("id",base64_decode($id))->update(["is_deleted"=>1]);

...
class CustomerController extends Controller{
    ...
    protected $slug         =   'users/customers';
    ...
    public function index(Request $request){
        ...
        $query          =   User::select("*")->where('role_id',3);
        ...
        return view('admin.customer.index',compact('counter','records'));
    }

    public function view($id,Request $request){
        $record    =   User::where("id",base64_decode($id))->first();
        if(!empty($record)){
            return view('admin.customer.view',compact('record'));
        }else{

					...
					<div class="box-body">
						{{ Form::open(array( "url" => $base_url, "method" => "get", "id"=>"search_form" )) }}
							...
							<div class="col-lg-2 col-md-4 col-sm-6 col-xs-12">
								...
								{{ Form::select("status",[ "all" => "All", "active" => "Active", "deactive" => "Deactivate" ,"unapproved"=>"Unapproved",],request()->status,array("class"=>"form-control")) }}
							</div>
										...
										<tr>
											...
											<td>
												...
												<!-- @if($record->is_approved == 1)
													<a class='btn btn-success' onclick='return confirm("Are you sure you want to disapprove this record?")' href="{{ url($base_url.'/approve_update/'.base64_encode($record->id).'/0').getUrlParams() }}" title="Dis-Approved">
														<i class='fa fa-check-circle'></i>
													</a>
												@else
													<a class='btn btn-danger' onclick='return confirm("Are you sure you want to approve this record?")' href="{{ url($base_url.'/approve_update/'.base64_encode($record->id).'/1').getUrlParams() }}" title="Activate">
